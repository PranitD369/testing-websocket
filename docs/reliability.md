...
- **Hybrid context replay** when the upstream model doesn't support `sessionResumption`. On every fresh Gemini WS, `Session.buildSeedContext(tail, summarizer)` splits the transcript into a `summary` (older turns **compressed by Gemini REST into a single paragraph**, cached on the `Session` and in `sessions.summary` so the next reconnect re-uses it until new older turns accrue) plus a `replay` array (the most recent `CONTEXT_REPLAY_TAIL_TURNS` turns, default 10, sent as one `clientContent` batch with `turnComplete:false` after `setupComplete`). The model gets long-range context (LLM summary) and live working context (replay) without depending on Gemini's native resumption — which the `gemini-2.5-flash-native-audio-*` family rejects. Summarizer timeouts (`SUMMARY_TIMEOUT_MS`, default 2.5s) fall back to a raw `role: text` concat so reconnect never stalls.
