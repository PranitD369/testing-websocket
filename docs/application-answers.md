...
1. **Server-side session state keyed by a client-owned id.** The browser persists its `sessionId` in `localStorage` and includes it on every `/ws` connect. The server's `SessionManager` looks up the same `Session` object across many WS connections (`src/session/SessionManager.ts:21`). Reconnect is no longer "new session" — it's "same session, new transport."

2. **Use the upstream API's resumption mechanism, not a homegrown replay.** Gemini Live exposes `sessionResumption` with `newHandle`/`resumable` semantics (`docs/gemini-notes.md`). Storing the handle on the `Session` and replaying it in the next setup means the *model* restores its own state — much cleaner than buffering and replaying turns.

3. **Treat `goAway` as a normal event, not an error.** Gemini sends `goAway.timeLeft` before forced disconnects (every ~10 minutes for video sessions). `UpstreamSupervisor.openReplacement` opens a parallel WS with the latest handle and swaps pointers when its `setupComplete` arrives (`src/reliability/reconnect.ts:85`). Client sees zero gap.
